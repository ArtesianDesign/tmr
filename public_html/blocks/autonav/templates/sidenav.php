...
$parent = Page::getByID($this->controller->displayPagesCID);
$pTitle = ($alt = $parent->getAttribute('alt_title')) ? $alt : $parent->getCollectionName();
...
*/
echo '<div class="nav side-nav">';
echo '<h4><a href="' . $nh->getLinkToCollection($parent) . '">' . $pTitle . '</a></h4>';
...
foreach ($navItems as $ni) {
	...
	if ($ni->hasSubmenu) {
		echo '<ul class="clearfix">'; //opens a dropdown sub-menu
	} else {
